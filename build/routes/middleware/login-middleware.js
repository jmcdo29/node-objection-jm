...
function alreadyLoggedIn(req, res, next) {
  if (req.user) {
    req.flash(
      'signedIn',
      'You are already signed in. Please log out to use a different account.'
    );
    res.redirect('/ideas');
  } else {
    next();
...
function verifyPass(req, res, next) {
  ...
  if (!/[!@#$%^&*]+?/.test(pass)) {
    error.push(
      'Your password does not contain a special character. Please make sure it does.'
    );
  }
  if (!/\d+?/.test(pass)) {
    error.push(
      'Your password does not contain a number. Please make sure it does.'
    );
  }
  if (!/[A-Z]+?/.test(pass)) {
    error.push(
      'Your password does not contain an uppercase character. Please make sure it does.'
    );
  }
  if (!/[a-z]+?/.test(pass)) {
    error.push(
      'Your password does not contain a lowercase character. Please make sure it doesn.'
    );
  }
  if (error.length !== 0) {
  ...
  User.query()
    ...
    .then(result => {
      if (result) {
        req.flash(
          'error',
          'Your new password cannot be the same as your previous one!'
        );
